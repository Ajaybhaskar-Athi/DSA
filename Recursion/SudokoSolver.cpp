...
public:
    ...
    bool solve(vector<vector<char>>&board){
        ...
        for(int row=0;row<board.size();row++){
            for(int col=0;col<board[0].size();col++){
                if(board[row][col]=='.'){ //if empty inside . no row or col got to inside if all filled .
    ...
    bool isValidPlace(vector<vector<char>>board,int row,int col,char c){
        for(int i=0;i<9;i++){ //iterate through row and columns
        //check rows
        if(board[row][i]==c)return false;
        if(board[i][col]==c)return false; //checking columns
